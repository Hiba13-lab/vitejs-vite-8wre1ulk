...
export default function AppExact(){
 ...
 return <><Toast/><div className="x-store">
  ...
  <section className={`x-products ${category==="Promotions"?"is-promotions":""}`} id="products"><div className="title"><h2>{category==="Accueil"&&!search?"Nos produits phares":search?`Résultats pour “${search}”`:category==="Promotions"?"Nos offres en promotion":category}</h2><button onClick={()=>{setCategory("Accueil");setSearch("")}}>Voir tous les produits →</button></div><div className="x-grid">{results.map(p=><article key={p.id}>{p.originalPrice&&category==="Promotions"&&<span className="promo-badge">PROMO</span>}<button className={`fav ${favorites.includes(p.id)?"liked":""}`} onClick={()=>setFavorites(f=>f.includes(p.id)?f.filter(x=>x!==p.id):[...f,p.id])}>♡</button><div className="pimg" onClick={()=>open(p)}><img src={p.image}/></div><div className="info"><h3 onClick={()=>open(p)}>{p.name}</h3><small>OSRAH</small>{p.originalPrice&&category==="Promotions"?<div className="promo-price"><span>{p.price.toFixed(0)} DH</span><del>{p.originalPrice.toFixed(0)} DH</del></div>:<b>{p.price.toFixed(0)} DH</b>}<button onClick={()=>add(p)}>Ajouter au panier</button></div></article>)}</div></section>
