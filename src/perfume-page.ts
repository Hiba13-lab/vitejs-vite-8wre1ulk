...
function renderPerfume(){
 ...
 document.querySelector('.tidy-men-page')?.remove();document.querySelector('.favorites-page')?.remove();document.querySelector('.perfume-page')?.remove();
 ...
 const favs=readFav();
 const page=document.createElement('main');page.className='perfume-page';
 ...
 page.querySelectorAll('[data-perfume-fav]').forEach(btn=>btn.addEventListener('click',()=>{const p=PERFUMES[Number((btn as HTMLElement).dataset.perfumeFav)];const favs=readFav();const idx=favs.findIndex(f=>norm(f.name)===norm(p.name));if(idx>=0)favs.splice(idx,1);else favs.push({name:p.name,image:p.image,price:`${p.price} DH`,brand:'TERRE D’ARÔMES'});writeFav(favs);renderPerfume()}));
}
