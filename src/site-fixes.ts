...
function fixSiteImages() {
  ...
  document.querySelectorAll<HTMLImageElement>('img').forEach(img => {
    if (img.dataset.osrahImageChecked === '1') return;
    img.dataset.osrahImageChecked = '1';
  ...
  document.querySelectorAll<HTMLButtonElement>('.x-cats button').forEach(button => {
    ...
    if (img && img.getAttribute('src') !== SOLAR_FALLBACK) img.src = SOLAR_FALLBACK;
  });
}
...
window.setTimeout(fixSiteImages, 1800);
