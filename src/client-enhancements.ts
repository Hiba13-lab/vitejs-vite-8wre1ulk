...
function refreshFavBadge(){
  const topButtons=Array.from(document.querySelectorAll<HTMLButtonElement>('.x-icons>button'));
  const fav=topButtons.find(b=>b.textContent?.includes('Favoris'));
  if(!fav)return;
  ...
  const n=readFavs().length; badge.textContent=String(n); badge.style.display=n?'grid':'none';
}

function toggleFavorite(btn:HTMLElement){
  const item=parseCard(btn);if(!item)return;
  let items=readFavs(); const i=items.findIndex(x=>x.name===item.name);
  if(i>=0)items.splice(i,1);else items.push(item);
  writeFavs(items); btn.classList.toggle('liked',i<0); refreshFavBadge();
}

function closeFavs(){document.querySelector('.favorites-panel-wrap')?.remove();document.body.classList.remove('favorites-open')}
function renderFavs(){
  closeFavs(); const items=readFavs(); document.body.classList.add('favorites-open');
  const wrap=document.createElement('div');wrap.className='favorites-panel-wrap';
  wrap.innerHTML=`<aside class="favorites-panel"><header><div><small>VOTRE SÉLECTION</small><h2>Mes favoris <sup>${items.length}</sup></h2></div><button class="favorites-close">×</button></header><div class="favorites-list">${items.length?items.map((p,i)=>`<article><div class="fav-img"><img src="${p.image}" alt="${p.name}"></div><div><small>${p.category}</small><h3>${p.name}</h3><b>${p.price}</b></div><button data-fav-remove="${i}" aria-label="Retirer">♡</button></article>`).join(''):`<div class="favorites-empty"><span>♡</span><b>Aucun favori pour le moment</b><p>Cliquez sur le cœur d’un produit pour le retrouver ici.</p></div>`}</div><footer><button class="favorites-continue">Continuer mes achats</button></footer></aside>`;
  document.body.appendChild(wrap);
  wrap.addEventListener('click',e=>{const t=e.target as HTMLElement;if(t===wrap||t.closest('.favorites-close')||t.closest('.favorites-continue')){closeFavs();return}const r=t.closest('[data-fav-remove]') as HTMLElement|null;if(r){const a=readFavs();a.splice(Number(r.dataset.favRemove),1);writeFavs(a);renderFavs();refreshFavBadge()}})
}
...
function markExistingFavorites(){
  ...
  document.querySelectorAll<HTMLElement>('.x-grid article').forEach(a=>{const n=a.querySelector('h3')?.textContent?.trim()||'';const h=a.querySelector<HTMLElement>('.fav');if(h)h.classList.toggle('liked',names.has(n))});
}

function enhance(){ensureLogout();refreshFavBadge();markExistingFavorites()}

document.addEventListener('click',e=>{
  const t=e.target as HTMLElement|null;if(!t)return;
  const heart=t.closest('.x-grid .fav') as HTMLElement|null;if(heart){toggleFavorite(heart);return}
  const topFav=t.closest('.x-icons>button') as HTMLElement|null;if(topFav&&topFav.textContent?.includes('Favoris')){e.preventDefault();e.stopPropagation();renderFavs();return}
},true);
