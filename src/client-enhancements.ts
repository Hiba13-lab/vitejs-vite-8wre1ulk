...
function refreshFavBadge(){
  const fav=Array.from(document.querySelectorAll<HTMLButtonElement>('.x-icons>button')).find(b=>b.textContent?.includes('Favoris'));
  if(!fav)return;
  let badge=fav.querySelector('.fav-count') as HTMLElement|null;
  const n=readFavs().length;
  if(!badge&&n){badge=document.createElement('b');badge.className='fav-count';fav.appendChild(badge)}
...
function markExistingFavorites(){
  ...
  document.querySelectorAll<HTMLElement>('.x-grid article').forEach(a=>{const n=a.querySelector('h3')?.textContent?.trim()||'';a.querySelector<HTMLElement>('.fav')?.classList.toggle('liked',names.has(n))});
}
...
document.addEventListener('click',e=>{
 ...
 const heart=t.closest('.x-grid .fav') as HTMLElement|null;if(heart){e.preventDefault();e.stopPropagation();toggleFavorite(heart);return}
...
// Refresh only after normal user navigation. Avoid MutationObserver here because
// enhance() itself changes the DOM and could create an endless observer loop.
